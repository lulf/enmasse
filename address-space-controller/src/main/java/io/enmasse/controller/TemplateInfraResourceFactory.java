...
public class TemplateInfraResourceFactory implements InfraResourceFactory {
    ...
    private void prepareParameters(InfraConfig infraConfig,
        ...
        String encodedCaCert = Optional.ofNullable(authService.getSpec().getCaSecretName())
                .map(secretName ->
        ...
        if (authService.getSpec().getClientSecretName() != null) {
            parameters.put(TemplateParameter.AUTHENTICATION_SERVICE_CLIENT_SECRET, authService.getSpec().getClientSecretName());
        }
        ...
        if (authService.getSpec().getRealm() != null) {
            ...
            parameters.put(TemplateParameter.AUTHENTICATION_SERVICE_SASL_INIT_HOST, KubeUtil.getAddressSpaceRealmName(addressSpace));
        }
