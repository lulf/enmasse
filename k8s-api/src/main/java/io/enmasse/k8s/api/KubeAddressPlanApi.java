...
public class KubeAddressPlanApi implements AddressPlanApi, ListerWatcher<AddressPlan, AddressPlanList> {
    ...
    public KubeAddressPlanApi(NamespacedOpenShiftClient client, CustomResourceDefinition addressSpacePlanDefinition) {
        this.client = client;
    ...
    public AddressPlanList list(ListOptions listOptions) {
        return client.customResources(addressSpacePlanDefinition, AddressPlan.class, AddressPlanList.class, DoneableAddressPlan.class).list();
    }
    ...
    public io.fabric8.kubernetes.client.Watch watch(io.fabric8.kubernetes.client.Watcher<AddressPlan> watcher, ListOptions listOptions) {
        ...
        return client.withRequestConfig(requestConfig).call(c ->
                c.customResources(addressSpacePlanDefinition, AddressPlan.class, AddressPlanList.class, DoneableAddressPlan.class).withResourceVersion(listOptions.getResourceVersion()).watch(watcher));
    }

    @Override
    public Watch watchAddressPlans(Watcher<AddressPlanList> watcher, Duration resyncInterval) {
        ...
        config.setProcessor(map -> {
            if (queue.hasSynced()) {
                AddressPlanList list = new AddressPlanList();
