        ...
        try {
            ...
            if (port != 0) {
                ...
                if (response.containsKey(node)) {
                    neighbours = filterOnAttribute(String.class, 0, response.get(node));
                }
                ...
                return new RouterStatus(router.getMetadata().getName(), connections, neighbours, undeliveredTotal);
            }
        } catch (Exception e) {
