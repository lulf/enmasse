...
public class KubeStandardInfraConfigApi implements StandardInfraConfigApi, ListerWatcher<StandardInfraConfig, StandardInfraConfigList> {
    ...
    public KubeStandardInfraConfigApi(NamespacedOpenShiftClient client, CustomResourceDefinition addressSpacePlanDefinition) {
        this.client = client;
    ...
    public StandardInfraConfigList list(ListOptions listOptions) {
        return client.customResources(addressSpacePlanDefinition, StandardInfraConfig.class, StandardInfraConfigList.class, DoneableStandardInfraConfig.class).list();
    }
    ...
    public io.fabric8.kubernetes.client.Watch watch(io.fabric8.kubernetes.client.Watcher<StandardInfraConfig> watcher, ListOptions listOptions) {
        ...
        return client.withRequestConfig(requestConfig).call(c ->
                c.customResources(addressSpacePlanDefinition, StandardInfraConfig.class, StandardInfraConfigList.class, DoneableStandardInfraConfig.class).withResourceVersion(listOptions.getResourceVersion()).watch(watcher));
    }

    @Override
    public Watch watchStandardInfraConfigs(Watcher<StandardInfraConfigList> watcher, Duration resyncInterval) {
        ...
        config.setProcessor(map -> {
            if (queue.hasSynced()) {
                StandardInfraConfigList list = new StandardInfraConfigList();
